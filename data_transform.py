...
def Process():
    ...
    tb_data.rename(columns={
                   "Estimated incidence of all forms of tuberculosis": "TB Cases"}, inplace=True)
    life_expectancy_data.rename(columns={
                                "Period life expectancy at birth - Sex: total - Age: 0": "Life Expectancy"}, inplace=True)

    merged_data = pd.merge(tb_data, life_expectancy_data, on=[
